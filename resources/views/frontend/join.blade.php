        ...
        <form action="{{ route('join.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" required
                           class="w-full rounded-md border-gray-300 shadow-sm border px-4 py-2 focus:border-red-500 focus:ring-red-500 @error('name') border-red-500 @enderror">
                    @error('name')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="email" class="block text-sm font-medium text-gray-700 mb-1">Alamat Email</label>
                    <input type="email" name="email" id="email" value="{{ old('email') }}" required
                           class="w-full rounded-md border-gray-300 shadow-sm border px-4 py-2 focus:border-red-500 focus:ring-red-500 @error('email') border-red-500 @enderror">
            ...
            <div>
                ...
                <input type="text" name="phone_number" id="phone_number" value="{{ old('phone_number') }}"
                       class="w-full rounded-md border-gray-300 shadow-sm border px-4 py-2 focus:border-red-500 focus:ring-red-500 @error('phone_number') border-red-500 @enderror">
            ...
            <div>
                ...
                <textarea name="reason" id="reason" rows="4" required placeholder="Ceritakan motivasi Anda bergabung dengan komunitas ini..."
                          class="w-full rounded-md border-gray-300 shadow-sm border px-4 py-2 focus:border-red-500 focus:ring-red-500 @error('reason') border-red-500 @enderror">{{ old('reason') }}</textarea>
