    ...
    <aside class="bg-gray-800 text-white w-64 min-h-screen fixed left-0 top-0 overflow-y-auto shadow-lg z-20">
        ...
        <nav class="mt-6 px-4">
            ...
            <ul class="space-y-2">
                ...
                <li>
                    <a href="{{ route('admin.users.index') }}" class="block px-4 py-2 rounded-md hover:bg-gray-700 transition-colors {{ request()->routeIs('admin.users.*') ? 'bg-gray-700 text-white' : 'text-gray-300' }}">Pengaturan Admin</a>
                </li>
                <li>
                    <a href="{{ route('admin.profile.edit') }}" class="block px-4 py-2 rounded-md hover:bg-gray-700 transition-colors {{ request()->routeIs('admin.profile.*') ? 'bg-gray-700 text-white' : 'text-gray-300' }}">Profil Saya</a>
                </li>
            </ul>
    ...
    <main class="ml-64 flex-1 flex flex-col min-h-screen">
        ...
        <header class="bg-white shadow-sm px-8 py-4 flex items-center justify-between">
            ...
            <div class="flex items-center space-x-4 text-gray-600">
                <a href="{{ route('admin.profile.edit') }}" class="hover:text-red-600 font-medium transition-colors flex items-center">
                    <svg class="w-5 h-5 mr-1 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                    Halo, <span class="font-bold ml-1">{{ auth()->user()->name ?? 'Admin' }}</span>
                </a>
            </div>
        </header>
