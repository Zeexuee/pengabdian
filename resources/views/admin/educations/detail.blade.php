...
@section('page_title', 'Kelola Detail Edukasi: ' . $education->title)
        ...
        if (sortableContainer) {
            Sortable.create(sortableContainer, {
                ...
                onEnd: function () {
                    ...
                    fetch('{{ url("admin/educations/" . $education->id . "/blocks/reorder") }}', {
                        method: 'POST',
                        headers: {
...
<div class="flex flex-wrap justify-between items-start gap-3 mb-6">
    <div>
        <a href="{{ route('admin.educations.index') }}" class="text-red-600 hover:underline text-sm font-semibold">
    ...
    <div class="lg:col-span-5">
        <div class="bg-gray-50 border border-gray-200 rounded-xl p-5 sticky top-6 shadow-sm">
            <h3 class="font-bold text-gray-800 text-base mb-4">
                Tambah Konten Detail Edukasi
            </h3>

            <form action="{{ route('admin.educations.blocks.store', $education->id) }}"
                ...
                <div id="field-text" class="hidden">
                    ...
                    <textarea name="content" id="block-content" rows="8"
                              class="w-full border-gray-300 rounded-lg shadow-sm text-sm focus:ring-red-500 focus:border-red-500"
                              placeholder="Tulis materi edukasi di sini…">{{ old('content') }}</textarea>
        ...
        <div id="sortable-blocks" class="space-y-4">
                ...
                <div data-id="{{ $block->id }}" class="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                    <div class="flex items-center justify-between px-4 py-3 bg-gray-50 border-b border-gray-200">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <form action="{{ route('admin.educations.blocks.destroy', [$education->id, $block->id]) }}"
                                  method="POST"
                    ...
                    <div id="edit-block-{{ $block->id }}" class="hidden p-4 border-b border-gray-200 bg-gray-50/80">
                        ...
                        <form action="{{ route('admin.educations.blocks.update', [$education->id, $block->id]) }}"
            ...
            @empty
                <div class="py-16 text-center bg-gray-50 rounded-xl border border-dashed border-gray-300">
                    <p class="text-gray-400 font-medium text-sm">Belum ada konten detail edukasi. Tambahkan dari panel kiri.</p>
                </div>
            @endforelse
        </div>
    </div>
