...
@section('content')
<div class="max-w-4xl">
    <form id="create-product-form" action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <textarea name="description" id="description_editor" rows="6"
                      class="w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                      placeholder="Tuliskan deskripsi, bahan, ukuran, atau informasi produk lainnya...">{{ old('description') }}</textarea>
        </div>

        {{-- Gambar Produk (Akumulatif) --}}
        <div>
            <label class="block text-gray-700 font-semibold mb-2">
                Gambar Produk
                <span class="text-gray-400 font-normal text-sm">(bisa tambah dari folder berbeda)</span>
            </label>
        ...
        <div class="border border-gray-200 rounded-lg p-4 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="shopee_url" id="shopee_url" value="{{ old('shopee_url') }}"
                       class="w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-400 text-sm @error('shopee_url') border-red-500 @enderror"
            ...
            <div>
                ...
                <input type="text" name="tokopedia_url" id="tokopedia_url" value="{{ old('tokopedia_url') }}"
        ...
        <div>
            ...
            <input type="text" name="video_url" id="video_url" value="{{ old('video_url') }}"
                   class="w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 @error('video_url') border-red-500 @enderror"
                   placeholder="https://www.youtube.com/watch?v=...">
            @error('video_url')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
            <p class="text-gray-400 text-xs mt-1">Masukkan URL YouTube atau Vimeo. Video akan ditampilkan di halaman detail produk.</p>
        </div>
        ...
        <div class="flex justify-end space-x-4 pt-4">
            <a href="{{ route('admin.products.index') }}" class="px-6 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 font-semibold transition">Batal</a>
            <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 font-semibold transition">Simpan Produk</button>
        </div>
    </form>
</div>
@endsection
...
<script>
    ...
    // Sync CKEditor ke textarea sebelum form submit
    ...
    let fileStore = new DataTransfer(); // kumpulan file yang dipilih

    document.getElementById('images-picker').addEventListener('change', function() {
        // Tambah file baru ke fileStore (tidak menghapus yang lama)
        for (let i = 0; i < this.files.length; i++) {
            fileStore.items.add(this.files[i]);
        }
        // Paksa input file memakai fileStore
        document.getElementById('images-real-input').files = fileStore.files;
        renderPreview();
        // Reset value agar onChange terpicu lagi jika pilih file yang sama
        this.value = '';
    });
    ...
    function renderPreview() {
        ...
        for (let i = 0; i < fileStore.files.length; i++) {
            ...
            reader.onload = function(e) {
                ...
                del.onclick = function() { removeFile(idx); };
