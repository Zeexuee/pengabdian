...
@section('page_title', 'Tambah Halaman (Block-Based)')
...
<div x-data="pageBuilder()">
    <form action="{{ route('admin.pages.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="title" id="title" value="{{ old('title') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2 border" required>
        </div>

        <div class="mb-4">
            <label for="slug" class="block text-sm font-medium text-gray-700">Slug (Opsional)</label>
            <input type="text" name="slug" id="slug" value="{{ old('slug') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2 border">
        ...
        <div class="mb-4 p-4 border rounded-md bg-white shadow-sm">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2 border" placeholder="Judul khusus untuk SEO (jika kosong, menggunakan Judul Halaman)">
            </div>
            <div class="mb-4">
                <label for="meta_description" class="block text-sm font-medium text-gray-700">Meta Description</label>
                <textarea name="meta_description" id="meta_description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2 border" placeholder="Deskripsi singkat untuk SEO"></textarea>
            </div>
            <div>
            ...
            <template x-for="(block, index) in blocks" :key="block.id">
                <div class="p-4 border rounded-md bg-gray-50 relative border-gray-200">
                    ...
                    <template x-if="block.type === 'gambar'">
                        <div>
                            <input type="file" :name="`content_blocks[${index}][content]`" accept="image/*" class="mt-1 block w-full text-sm text-gray-900 border border-gray-300 rounded-md cursor-pointer bg-gray-50 p-2" required>
                            <p class="mt-1 text-sm text-gray-500">Pilih file gambar untuk diunggah.</p>
                        </div>
                    </template>
                    ...
                    <template x-if="block.type === 'video_youtube'">
                        <div>
                            ...
                            <p class="text-xs text-gray-500 mt-1">Hanya ID video, bukan URL penuh (cth: dari youtube.com/watch?v=ID_VIDEO ambil ID_VIDEO-nya).</p>
                        </div>
                    </template>
                    ...
                    <template x-if="block.type === 'tabel'">
                        <div>
                            ...
                            <p class="text-xs text-gray-500 mt-1">Gunakan format CSV sederhana (pisahkan kolom dengan koma, baris baru untuk baris tabel). Baris pertama otomatis menjadi Header (Judul Kolom).</p>
                        </div>
                    </template>

                    <template x-if="block.type === 'file'">
                        <div>
                            <input type="file" :name="`content_blocks[${index}][content]`" accept=".pdf,.doc,.docx" class="mt-1 block w-full text-sm text-gray-900 border border-gray-300 rounded-md cursor-pointer bg-gray-50 p-2" required>
                            <p class="mt-1 text-sm text-gray-500">Pilih dokumen untuk diunggah (PDF, DOC, DOCX).</p>
                        </div>
                    </template>
                    ...
                    <template x-if="block.type === 'grup_teks_gambar'">
                        <div class="flex flex-col md:flex-row md:space-x-4 space-y-4 md:space-y-0">
                            ...
                            <div class="flex-1">
                                ...
                                <input type="file" :name="`content_blocks[${index}][content][gambar]`" accept="image/*" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-md cursor-pointer bg-gray-50 p-2" required>
                                <p class="mt-1 text-xs text-gray-500">Pilih gambar pendamping teks.</p>
                            </div>
                        </div>
                    </template>
                    ...
                    <template x-if="block.type === 'video'">
                        <div>
                            <input type="file" :name="`content_blocks[${index}][content]`" accept="video/mp4,video/webm" class="mt-1 block w-full text-sm text-gray-900 border border-gray-300 rounded-md cursor-pointer bg-gray-50 p-2" required>
                            <p class="mt-1 text-sm text-gray-500">Pilih file video (MP4, WebM).</p>
                        </div>
                    </template>

                    <template x-if="block.type === 'slider'">
                        <div>
                            <input type="file" :name="`content_blocks[${index}][content][]`" accept="image/*" multiple class="mt-1 block w-full text-sm text-gray-900 border border-gray-300 rounded-md cursor-pointer bg-gray-50 p-2" required>
                            <p class="mt-1 text-xs text-red-500">Maksimal 5 gambar sekaligus.</p>
                        </div>
                    </template>
                    ...
                    <template x-if="block.type === 'grup_gambar'">
                        <div>
                            <input type="file" :name="`content_blocks[${index}][content][]`" accept="image/*" multiple class="mt-1 block w-full text-sm text-gray-900 border border-gray-300 rounded-md cursor-pointer bg-gray-50 p-2" required>
                            <p class="mt-1 text-sm text-gray-500">Pilih gambar (unggah banyak file sekaligus) untuk disejajarkan.</p>
                        </div>
                    </template>
