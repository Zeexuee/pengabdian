...
@section('page_title', 'Tambah Divisi Baru')
    ...
    <form action="{{ route('admin.divisions.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" id="name" value="{{ old('name') }}" required 
                ...
                placeholder="Contoh: Pengurus Harian, Divisi Komunikasi & Informasi, Divisi Pengabdian">
            @error('name')
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" rows="3" 
                class="w-full border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-sm"
                placeholder="Tuliskan gambaran singkat peran divisi ini...">{{ old('description') }}</textarea>
            @error('description')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            <label for="banner_image" class="block text-sm font-medium text-gray-700 mb-1">Upload Gambar Hero Banner Divisi</label>
            ...
            <p class="text-gray-500 text-xs mt-1">Gambar ini akan tampil sebagai banner paling atas saat divisi ini dibuka di website. Format: JPG, PNG, WEBP (Max: 3MB).</p>
            @error('banner_image')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
